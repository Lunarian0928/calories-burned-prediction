...
#### 도메인 피처 유효성 검증 및 중요도 분석 (Domain Features Validation & Importance Anaylsis)
<img width="1790" height="1390" alt="image" src="https://github.com/user-attachments/assets/dcbad40d-cd11-4d50-8fbd-e8531cb94384" />

최종 스태킹 모델을 구성하는 4개 1차 모델(XGBoost, LightGBM, CatBoost, Random Forest)의  
평균 피처 중요도를 산출하여 파생 변수의 예측 기여도를 분석하였습니다.

원본 독립 변수보다 의료 도메인 지식 기반의 파생 변수들이 상위권을 차지하며,  
피처 엔지니어링의 논리적 타당성을 수치로 증명하였습니다. 

- **물리적 운동 부하 변수의 압도적 기여도 (Physical Workload)**:
운동 시간과 체중을 곱한 교호작용 변수인 `Inter_Exercise_Duration_x_Weight`가 평균 중요도 874.78로 전체 1위를 기록했습니다.  
이는 물리적인 질량(체중)이 지속해서 움직인 시간(운동 시간)이 칼로리 소모를 결정짓는 가장 핵심적인 요인이라는 것으로,  
모델이 최우선으로 학습했음을 의미합니다.

- **의학 및 열역학 도메인 가설의 성공적 입증 (Doamin Knowledge Validation)**:
체질량 지수인 `BMI`(2위, 666.01), 열에너지 산출 지표인 `Heat_Energy_Proxy`(5위, 497.31),  
기본 공식 예측치인 `Session_Keytel_Calories`(6위, 463.80) 등  
사전에 도메인 논리를 바탕으로 설계한 파생 변수들이 최상위권에 올랐습니다.  
이를 통해 의학적 기반에 근거한 피처 설계가 모델의 잔차 예측 성능을 극대화하는 데 직접적으로 작용했음을 확인할 수 있습니다.

- **생리적 반응과 신체 조건의 복합 패턴 포착 (Biometric Interactions)**:
심박수 및 체온 수치를 키(Height)에 결합한 `Inter_BPM_x_height`(3위), `Inter_Body_Temperature_x_Height`(8위)와 더불어  
심박 부하율을 나타내는 `HR_Load_Rate`(7위)가 높은 중요도를 보였습니다.  
이는 모델이 단일 생체 지표로는 설명할 수 없는,  
개인의 신체 크기에 따른 심혈관계 및 체온 변화의 비선형적 상호작용을 오차 보정의 주요 기준으로 활용했음을 보여줍니다.
